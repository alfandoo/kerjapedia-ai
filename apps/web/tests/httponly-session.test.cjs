...
function load(responses = [], production = true, maxBody = 50 * 1024 * 1024) {
  ...
  const calls = [];
  const source = fs
    .readFileSync(path.join(__dirname, "../src/app/api/backend/[...path]/route.ts"), "utf8")
    .replace("const MAX_BODY = 50 * 1024 * 1024;", `const MAX_BODY = ${maxBody};`);
  vm.runInNewContext(
    ts.transpileModule(source, { compilerOptions: { module: ts.ModuleKind.CommonJS } }).outputText,
    {
      ...
      process: {
        env: {
          ...
          API_INTERNAL_URL: "http://private-api.test:8000",
      ...
      fetch: async (url, options) => {
        ...
        if (options.body instanceof ReadableStream)
          calls[calls.length - 1].body = await new Response(options.body).text();
  ...
  return {
    calls,
    run: async (route, method = "POST", headers = {}, body) => {
      const req = new NextRequest("https://app.example.test/api/backend/" + route, {
        method,
...
const tokens = () =>
  json({
    user: { ...user, refresh_token: "nested-secret" },
...
test("login returns only profile and sets host-only Secure HttpOnly cookies", async () => {
  ...
  const result = await h.run(
    ...
    "POST",
    {},
    { email: user.email, password: "password", access_token: "forged" }
  );
...
for (const headers of [
  ...
  test("CSRF rejected before backend call: " + JSON.stringify(headers), async () => {
    const h = load();
    const result = await h.run("auth/login", "POST", headers, {
      email: user.email,
      password: "password",
    });
...
test("guest requests retain guest identity and have no bearer", async () => {
  const h = load([json([])]);
  await h.run("chat/conversations", "GET", {
    cookie: "__Host-kp-guest=11111111-1111-4111-8111-111111111111",
    "x-kerjapedia-guest-id": "forged",
    authorization: "Bearer forged",
  });
  assert.equal(h.calls[0].options.headers.get("authorization"), null);
  assert.equal(
    h.calls[0].options.headers.get("x-kerjapedia-guest-id"),
...
test("path traversal, unknown auth routes and GET login are rejected", async () => {
  const h = load();
  for (const target of ["admin/../auth/login", "auth/unknown", "metrics"])
    assert.equal((await h.run(target)).status, 404);
...
test("guest identity is generated in an HttpOnly cookie, ignoring forged headers", async () => {
  ...
  for (const value of ["__Host-kp-guest=" + guest, "HttpOnly", "Secure", "SameSite=lax"])
    assert.ok(cookie.includes(value));
});
...
test("profile endpoint rejects guests and privileged profile fields", async () => {
  ...
  assert.equal(
    (await h.run("auth/profile", "PATCH", { cookie }, { name: "Name", roles: ["admin"] })).status,
    400
  );
  assert.equal(
    (
