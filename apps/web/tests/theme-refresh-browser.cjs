...
(async () => {
  ...
  try {
    ...
    await context.route("**/*", route => {
      ...
      if (url.pathname.startsWith("/api/backend/")) return route.fulfill({ status: url.pathname.includes("/auth/") ? 401 : 200, contentType: "application/json", body: url.pathname.includes("/auth/") ? "{}" : "[]" });
    ...
    async function choose(theme) {
      ...
      await page.locator('#settings-appearance-options').getByRole("button", { name: theme, exact: true }).click();
    }
