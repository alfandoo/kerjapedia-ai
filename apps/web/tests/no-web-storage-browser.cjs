...
(async () => {
 ...
 try {
  const context = await browser.newContext({ locale: "en-US", viewport: { width: 1584, height: 960 } });
  await context.addInitScript(() => {
   for (const name of ["localStorage", "sessionStorage"]) Object.defineProperty(window, name, { get() { throw Error("Web Storage forbidden"); } });
  });
  let owner = "owner-one";
  await context.route("**/*", route => {
   const url = new URL(route.request().url());
   if (url.origin !== new URL(base).origin) return route.abort();
   if (url.pathname === "/api/backend/auth/session") return route.fulfill({ json: { user: { user_id: owner, name: "Test Owner", email: "owner@example.test", roles: ["user"] } } });
   if (url.pathname === "/api/backend/chat/conversations") return route.fulfill({ json: [{ conversation_id: "pin-test", title: "Pin persistence test", created_at: "2026-09-05T00:00:00Z", updated_at: "2026-09-05T00:00:00Z", message_count: 2 }] });
  ...
  await expect(page.getByRole("button", { name: "Unpin chat", exact: true }).first()).toBeVisible();
  await page.reload();
  await expect(page.getByRole("button", { name: "Unpin chat", exact: true }).first()).toBeVisible();
  assert.equal((await context.cookies()).find(c => /kp-guest$/.test(c.name)).value, guest.value);
  ...
  assert.equal((await context.cookies()).find(c => c.name === "kerjapedia.chat.sidebar.v1").value, "collapsed");
