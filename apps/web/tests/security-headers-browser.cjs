...
(async () => {
  const server = spawn(process.execPath, ["node_modules/next/dist/bin/next", "start", "--hostname", "127.0.0.1", "--port", "3103"], {
    env: { ...process.env, NEXT_DIST_DIR: ".next-security-headers", API_INTERNAL_URL: "http://127.0.0.1:1", APP_ORIGIN: base }, windowsHide: true, stdio: ["ignore", "pipe", "pipe"],
  });
  let output = "";
  ...
  try {
    ...
    await page.route("**/*", route => {
      ...
      if (url.pathname.startsWith("/api/backend/")) return route.fulfill({
        status: url.pathname.includes("/auth/") ? 401 : 200,
        contentType: "application/json", body: url.pathname.includes("/auth/") ? '{"detail":"No mock session"}' : '[]',
      });
      return route.continue();
    });
    const violations = [];
    page.on("console", msg => { if (/violates.*Content Security Policy|Refused to.*(script|style)/i.test(msg.text())) violations.push(msg.text()); });
    const nonces = [];
    for (const path of ["/", "/login-admin", "/admin/dashboard", "/search", "/legal/privacy"]) {
      ...
      assert.ok(scripts.length > 0 && scripts.every(tag => tag.includes(`nonce="${nonce}"`)), "Server-rendered scripts must carry the response nonce");
    ...
    assert.equal(await page.getByRole("button", { name: "Sembunyikan kata sandi" }).getAttribute("aria-pressed"), "true");
    ...
    await page.route(base + "/login-admin", async route => {
      ...
      const body = (await original.text()).replace("</body>", "<script>window.__cspInjected = true</script></body>");
