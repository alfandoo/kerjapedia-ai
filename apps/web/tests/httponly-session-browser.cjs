...
const user = { user_id: "mock-admin", email: "admin@example.test", name: "Admin Cookie Test", roles: ["admin"] };
(async () => {
  let access = "mock-access-1", refresh = "mock-refresh-1", revoked = false, refreshCount = 0;
  const calls = [];
  const fakeApi = http.createServer(async (req, res) => {
    ...
    if (req.url === "/auth/login") { revoked = false; return reply(200, { user, access_token: access, refresh_token: refresh }); }
    if (req.url === "/auth/refresh") {
      if (revoked || JSON.parse(body).refresh_token !== refresh) return reply(401, {});
      refreshCount++; access = "mock-access-" + (refreshCount + 1); refresh = "mock-refresh-" + (refreshCount + 1);
      return reply(200, { user, access_token: access, refresh_token: refresh });
    }
    if (req.url === "/auth/me") return reply(!revoked && req.headers.authorization === "Bearer " + access ? 200 : 401, user);
    if (req.url === "/auth/logout") { revoked = true; return reply(200, { status: "ok" }); }
    if (req.url.startsWith("/admin")) return reply(503, { detail: "Mock admin dataset unavailable" });
    if (req.url === "/chat/ask/stream") {
      res.writeHead(200, { "content-type": "application/x-ndjson" }); res.write('{"event":"delta","content":"hello"}\n');
      return setTimeout(() => res.end('{"event":"done"}\n'), 20);
    }
    if (req.url === "/feedback") return reply(200, { feedback_id: "mock-feedback" });
    return reply(200, []);
  });
  await new Promise((resolve, reject) => { fakeApi.once("error", reject); fakeApi.listen(3102, "127.0.0.1", resolve); });
  const server = spawn(process.execPath, ["node_modules/next/dist/bin/next", "start", "--hostname", "127.0.0.1", "--port", "3101"], {
    env: { ...process.env, NEXT_DIST_DIR: ".next-security-headers", API_INTERNAL_URL: "http://127.0.0.1:3102", APP_ORIGIN: base }, windowsHide: true, stdio: ["ignore", "pipe", "pipe"],
  });
  ...
  try {
    ...
    for (let i = 0; i < 80; i++) {
      ...
      try { ready = (await fetch(base + "/login-admin")).ok; } catch {}
      if (ready) break;
    ...
    const stream = await page.evaluate(async () => {
      const response = await fetch("/api/backend/chat/ask/stream", { method: "POST", headers: { "X-KerjaPedia-CSRF": "1", "Content-Type": "application/json" }, body: JSON.stringify({ question: "mock" }) });
      return { status: response.status, body: await response.text() };
    });
    assert.equal(stream.status, 200); assert.ok(stream.body.includes("hello"));
    const missingCsrf = await page.evaluate(async () => (await fetch("/api/backend/feedback", { method: "POST", body: "{}" })).status);
    assert.equal(missingCsrf, 403);
    const forgedOrigin = await fetch(base + "/api/backend/auth/logout", { method: "POST", headers: { Origin: "https://evil.test", "X-KerjaPedia-CSRF": "1" } });
    assert.equal(forgedOrigin.status, 403); assert.equal(revoked, false);
    ...
    if (process.env.RUN_EXISTING_E2E === "1") {
      ...
      const code = await new Promise((resolve, reject) => {
        const runner = spawn(process.execPath, ["node_modules/@playwright/test/cli.js", "test", "--workers=2", "--timeout=20000", "--max-failures=3"], {
          env: { ...process.env, PLAYWRIGHT_BASE_URL: base }, windowsHide: true, stdio: "inherit",
        });
    ...
    console.log("UPSTREAM PATHS", calls.map(c => c.path));
