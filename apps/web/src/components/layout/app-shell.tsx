...
import { ChatIcon, DatabaseIcon, FileIcon, ScaleIcon, SearchIcon } from "@/components/icons";
import { ConversationHistory } from "@/features/chat/components/conversation-history";
import { useStoredSession } from "@/features/auth/session";
    ...
    <div className="min-h-[100svh] bg-white text-tinta">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[240px_minmax(0,1fr)] min-[1181px]:grid-cols-[300px_minmax(0,1fr)_360px]">
        ...
        >
          ...
          <div className="flex flex-col gap-1.5 max-lg:hidden lg:flex-1" aria-busy={historyLoading}>
            {historyLoading ? (
            ...
            {conversations.map((item) => (
              <button
                ...
                className={`flex min-h-14 w-full items-center gap-2.5 rounded-[7px] border-0 bg-transparent px-3 py-2.5 text-left text-tinta transition hover:translate-x-0.5 hover:bg-[#f7f7f8] ${
                  item.conversation_id === activeConversationId ? "bg-[#dff4f1] text-forest" : ""
                }`}
