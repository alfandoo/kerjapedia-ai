...
import { createIngestionJob, fetchIngestionJob, fetchIngestionJobs } from "@/lib/api";
    ...
    pollingRef.current = setInterval(async () => {
      try {
        ...
        if (updated.status !== "running" && updated.status !== "queued") {
          ...
          setJobs((current) =>
            current.map((j) => (j.job_id === jobId ? { ...j, ...updated } : j))
          );
          toast.success(
        ...
        {[
          ...
          <Card key={stat.label} className="py-0">
            <CardContent className="flex items-center gap-3 py-4">
              <span className={cn("flex size-10 shrink-0 items-center justify-center rounded-xl", stat.bg)}>
                ...
                <CardContent className="space-y-5">
                    ...
                    <div className="space-y-1.5">
                      {[
                        { label: "PDF diekstrak", done: selectedJob.status !== "queued" },
                        {
                          ...
                          done: selectedJob.status === "completed" || selectedJob.status === "needs_review" || selectedJob.status === "failed",
                        },
                        {
                        ...
                        <div key={stage.label} className="flex items-center gap-2 text-sm">
                          <span
                            className={cn(
                              ...
                              stage.done ? "bg-forest text-white" : "bg-surface-soft text-muted-text"
                            )}
                          >
                            {stage.done ? (
                  ...
                  <dl className="grid grid-cols-2 gap-x-4 gap-y-3">
                    <div className="min-w-0">
                      ...
                      <dd className="truncate font-mono text-sm font-medium">{selectedJob.job_id}</dd>
                    </div>
                    <div className="min-w-0">
