          ...
          <div className="flex-1 p-5">
            ...
            {section === "appearance" ? (
              <div className="space-y-3">
                <p className="text-[13px] font-medium text-muted-text">Mode tampilan</p>
                <div className="rounded-2xl border border-[#e8e4dc] bg-[#f9f9f9]">
                  {([
                    { value: "system" as const, label: "Sistem", desc: "Mengikuti sistem operasi" },
                    { value: "dark" as const, label: "Gelap", desc: "Mode gelap" },
                    { value: "light" as const, label: "Terang", desc: "Mode terang" },
                  ]).map((option, index) => (
                    <div key={option.value}>
                      {index > 0 ? <div className="border-t border-[#e8e4dc]" /> : null}
                      <button
                        type="button"
                        className={`flex min-h-[60px] w-full items-center gap-4 px-4 text-left transition hover:bg-[#f0f2f0] ${
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <span className="block text-[15px] font-medium text-tinta">{option.label}</span>
                          <span className="block text-[13px] text-muted-text">{option.desc}</span>
                        </div>
                        {theme === option.value ? (
                          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="size-5 shrink-0 text-javanese">
            ...
            {section === "language" ? (
              <div className="space-y-3">
                <p className="text-[13px] font-medium text-muted-text">Pilih bahasa</p>
                <div className="rounded-2xl border border-[#e8e4dc] bg-[#f9f9f9]">
                  {([
                    { value: "auto" as const, label: "Otomatis", desc: "Deteksi otomatis dari sistem" },
                    { value: "id" as const, label: "Indonesia", desc: "Bahasa Indonesia" },
                    { value: "en" as const, label: "English", desc: "English" },
                  ]).map((option, index) => (
                    <div key={option.value}>
                      {index > 0 ? <div className="border-t border-[#e8e4dc]" /> : null}
                      <button
