...
import { AlertIcon, ExternalIcon, SearchIcon } from "@/components/icons";
...
export function RegulationSearch() {
  ...
  const years = useMemo(
    () => [
      "all",
      ...Array.from(new Set(documents.map((document) => document.year)))
        .sort()
        .reverse(),
    ],
    [documents]
  ...
  return (
    <section className="w-full">
      ...
      <div
        className="grid min-h-16 grid-cols-[1fr_auto] items-center gap-2 border-b border-[#e5e5e5] py-4"
        aria-live="polite"
      >
        <div>
      ...
      <div aria-label="Daftar regulasi" aria-busy={loading}>
        ...
        {!loading && filtered.length > 0 ? (
          <div
            className="hidden grid-cols-[170px_minmax(280px,1fr)_70px_130px_150px] items-center gap-5 border-b border-[#e5e5e5] py-3 text-[10px] font-semibold uppercase tracking-[0.08em] text-slate-500 md:grid"
            aria-hidden="true"
          >
