...
function publicUser(value: unknown) {
  ...
  if (
    ![user.user_id, user.email, user.name].every((x) => typeof x === "string") ||
    !Array.isArray(user.roles) ||
    !user.roles.every((x) => typeof x === "string")
  )
    return null;
...
async function handle(request: NextRequest, context: Context): Promise<Response> {
  ...
  if (
    !path.length ||
    !roots.has(path[0]) ||
    path.some((p) => !/^[a-zA-Z0-9_-]+(?:\.[a-zA-Z0-9_-]+)*$/.test(p))
  )
  ...
  if (!["GET", "HEAD", "OPTIONS"].includes(request.method)) {
    ...
    if (
      ...
      request.headers.get("x-kerjapedia-csrf") !== "1" ||
  ...
  if (auth && action === "refresh" && !refresh)
    return clearCookies(json({ detail: "Session expired." }, 401));
  if (auth && ["session", "me", "profile", "account"].includes(action) && !access)
    return json({ detail: "No active session." }, 401);
  if (auth && action === "logout" && !access) {
    // A refresh cookie may still be valid: let the client refresh then revoke.
    return refresh
      ? json({ detail: "Session refresh required." }, 401)
      : clearCookies(json({ status: "ok" }));
  }
  ...
  try {
    ...
    const targetPath =
      auth && action === "session" ? "auth/me" : path.map(encodeURIComponent).join("/");
    ...
    if (
      ...
      !(
        auth &&
        ["login", "register", "refresh", "google", "verify-email-otp", "resend-otp"].includes(
          action
        )
      )
    )
    ...
    if (auth && ["login", "register"].includes(action)) {
      ...
      const input = await smallJson(request);
      const allowed = action === "verify-email-otp" ? ["email", "token"] : ["email"];
      if (
        !input ||
        typeof input !== "object" ||
        Array.isArray(input) ||
        ...
        return json({ detail: "Invalid verification payload." }, 400);
      if (
        typeof input.email !== "string" ||
        !input.email ||
        (action === "verify-email-otp" && (typeof input.token !== "string" || !input.token))
      )
        return json({ detail: "Invalid verification payload." }, 400);
      body = JSON.stringify(
      ...
      if (
        ...
        typeof input !== "object" ||
        Array.isArray(input) ||
        Object.keys(input).some((key) => key !== "id_token") ||
        typeof input.id_token !== "string" ||
        !input.id_token
      )
        return json({ detail: "Invalid Google credential." }, 400);
      ...
      if (
        ...
        return json({ detail: "Invalid profile fields." }, 400);
      ...
      if (declared && (!/^\d+$/.test(declared) || Number(declared) > MAX_BODY))
        return json({ detail: "Upload exceeds the 50 MB limit." }, 413);
      let size = 0;
      body =
    ...
    if (upstream.status >= 300 && upstream.status < 400)
      return json({ detail: "Unexpected backend redirect." }, 502);
    if (auth) {
      if (!upstream.ok) {
          ...
          : json(
              {
                detail:
                  upstream.status === 401
                    ? "Sesi berakhir atau kredensial tidak valid."
                    : "Permintaan autentikasi gagal. Silakan coba lagi.",
              },
              upstream.status
            );
      ...
      if (action === "register" && !result.access_token)
        return json({ requires_email_confirmation: true }, 202);
      if (
        !user ||
        ...
        return json({ detail: "Invalid session response." }, 502);
    ...
    if (guest && !validGuest)
      response.cookies.set(GUEST, guest, { ...cookieOptions, maxAge: 31536000 });
    return response;
  } catch (error) {
    if (oversized || (error instanceof Error && error.message === "body_limit"))
